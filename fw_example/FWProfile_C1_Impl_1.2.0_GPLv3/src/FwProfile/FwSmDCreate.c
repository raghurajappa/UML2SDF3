...
FwSmDesc_t FwSmCreate(FwSmCounterS1_t nOfStates, FwSmCounterS1_t nOfChoicePseudoStates,
	...
	if (nOfStates > 0) {
		smBase->pStates = (SmPState_t*) malloc((nOfStates)*sizeof(SmPState_t));
		if (smBase->pStates == NULL)
			return NULL;
		for (i=0; i<nOfStates; i++)
			smBase->pStates[i].outTransIndex = 0;
		smDesc->esmDesc = (struct FwSmDesc**) malloc((nOfStates)*sizeof(FwSmDesc_t));
	...
	if (nOfChoicePseudoStates > 0) {
		smBase->cStates = (SmCState_t*) malloc((nOfChoicePseudoStates)*sizeof(SmCState_t));
	...
	smBase->trans = (SmTrans_t*) malloc((nOfTrans)*sizeof(SmTrans_t));
	if (smBase->trans == NULL)
		return NULL;
	for (i=0; i<nOfTrans; i++)
		smBase->trans[i].iTrAction = -1;

	smDesc->smActions = (FwSmAction_t*) malloc((nOfActions+1)*sizeof(FwSmAction_t));
	...
	smDesc->smGuards = (FwSmGuard_t*) malloc((nOfGuards+1)*sizeof(FwSmGuard_t));
	...
	smDesc->smData = NULL;
	smDesc->transCnt = 1;
	smDesc->nOfActions = nOfActions+1;
	smDesc->nOfGuards = nOfGuards+1;
	smDesc->smExecCnt = 0;
...
FwSmDesc_t FwSmCreateDer(FwSmDesc_t smDesc) {
	...
	if (smBase->nOfPStates > 0) {
		extSmDesc->esmDesc = (struct FwSmDesc**) malloc((smBase->nOfPStates)*sizeof(FwSmDesc_t));
	...
	extSmDesc->smActions = (FwSmAction_t*) malloc((smDesc->nOfActions)*sizeof(FwSmAction_t));
	...
	extSmDesc->smGuards = (FwSmGuard_t*) malloc((smDesc->nOfGuards)*sizeof(FwSmGuard_t));
