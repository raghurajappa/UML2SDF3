...
FwPrDesc_t FwPrCreate(FwPrCounterS1_t nOfANodes, FwPrCounterS1_t nOfDNodes,
	...
	prBase->aNodes = (PrANode_t*) malloc((nOfANodes)*sizeof(PrANode_t));
	...
	if (nOfDNodes > 0) {
		prBase->dNodes = (PrDNode_t*) malloc((nOfDNodes)*sizeof(PrDNode_t));
	...
	prBase->flows = (PrFlow_t*) malloc((nOfFlows)*sizeof(PrFlow_t));
	if (prBase->flows == NULL)
		return NULL;
	for (i=0; i<nOfFlows; i++)
		prBase->flows[i].iGuard = -1;

	prDesc->prActions = (FwPrAction_t*) malloc((nOfActions)*sizeof(FwPrAction_t));
	if (prDesc->prActions == NULL)
		return NULL;
	for (i=0; i<nOfActions; i++)
		prDesc->prActions[i] = NULL;

	prDesc->prGuards = (FwPrGuard_t*) malloc((nOfGuards+1)*sizeof(FwPrGuard_t));
...
FwPrDesc_t FwPrCreateDer(FwPrDesc_t prDesc) {
	...
	extPrDesc->prActions = (FwPrAction_t*) malloc((prDesc->nOfActions)*sizeof(FwPrAction_t));
	...
	extPrDesc->prGuards = (FwPrGuard_t*) malloc((prDesc->nOfGuards)*sizeof(FwPrGuard_t));
